...
const AddNewPaletteModal = ({ navigation }) => {
  //modal is a top level component so we can access navigation
  ...
  }, [name, selectedColors]); // every time name changes, useCallback should run again, if name was not a dependency, useCallback
  // would rely on the initial name defined

  const handleUpdate = useCallback((value, selectedColor) => {
    if (value) {
      return setSelectedColors((prevColors) => [...prevColors, selectedColor]);
    }
    setSelectedColors((prevColors) =>
      prevColors.filter((color) => selectedColor.name !== color.name)
    );
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* we can also use onChange; we can also onChangeText={value => setName(value)} but as we are passing the 
    arg straight through we can omit this step*/}
